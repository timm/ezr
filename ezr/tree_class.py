...
from ez_class import (BIG, Data, Num, Sym, the, eg_B, eg_C,eg_s,eg_S,
...
def evaluate(d:Data, trainer:callable, repeats=20, file="", eps=None) -> Num:
  half = len(d.rows) // 2
  Y    = lambda r: d.disty(r)
  
  # Create Num large enough to hold all rows
  b4 = Num(mx=len(d.rows))
  for r in d.rows: b4.add(Y(r))
  
  # Use passed-in eps value, or calculate if not provided
  if eps is None:
    eps = 0.35 * b4.spread()
  ...
  for _ in range(repeats):
    rows  = shuffle(d.rows)
    test  = rows[half:]
    train = trainer(d, rows[:half])
    t     = Tree(clone(d, train), train)
    used += len(treeUsed(t, d))
    test.sort(key=lambda r: treeLeaf(t, r).y.mid())
    wins.add(win(min(test[:the.Check], key=Y)))
  parts = [f"{round(wins.mid())}"
           f" ,trainer {trainer.__name__}"
           f" ,sd {round(wins.spread())}"
           f" ,b4 {say(b4[half])}"
           f" ,lo {say(b4[0])}"
           f" ,B {len(train)}"
           f" ,C {the.Check}"
           f" ,U {int(used/repeats)}"
           f" ,eps {say(eps)}",
           *[f"{s} {len(a)}" for s,a in
             dict(x=d.cols.x, y=d.cols.y, r=d.rows).items()]]
  if file: parts.extend(file.split("/")[-2:])
  print(*parts, sep=" ,")
  return wins
  
def random_trainer(d:Data, rows:Rows) -> Rows:
  """Baseline: random sampling"""
  return rows[:the.Budget]
...
def eg__test(f:filename):
  "test tree-guided optimization with random sampling"
  evaluate(Data(csv(f)), random_trainer, file=f)
